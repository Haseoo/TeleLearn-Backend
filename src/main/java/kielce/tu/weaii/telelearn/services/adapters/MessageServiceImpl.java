...
public class MessageServiceImpl implements MessageService {
    ...
    private List<ConversationInfo> prepareConversationInfo(Long userId) {
        ...
        for (Map.Entry<User, List<Message>> entry : getConversationsMap(userId).entrySet()) {
            long unreadMessages = entry.getValue()
                    .stream()
    ...
    private Map<User, List<Message>> getConversationsMap(Long userId) {
        User user = userService.getById(userId);
        List<Message> userMessage = messageRepository.getUserMessages(userId);

        Map<User, List<Message>> formUserMessages = userMessage.stream()
                .collect(Collectors.groupingBy(Message::getSender));
        formUserMessages.remove(user);

        Map<User, List<Message>> toUserMessage = userMessage.stream()
                .collect(Collectors.groupingBy(Message::getReceiver));
        toUserMessage.remove(user);

        return Stream.concat(formUserMessages.entrySet().stream(), toUserMessage.entrySet().stream())
