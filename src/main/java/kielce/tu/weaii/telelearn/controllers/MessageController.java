...
import kielce.tu.weaii.telelearn.services.adapters.MessageServiceImpl;
...
public class MessageController {
    ...
    @GetMapping(path = "/{id}")
    public ResponseEntity<List<ConversationInfoView>> getConversations(@PathVariable Long id) {
        return new ResponseEntity<>(messageService.getConversations(id)
                .stream()
    ...
    @PutMapping
    public ResponseEntity<Object> sendMessage(@Valid @RequestBody SendMessageRequest request) {
        messageService.sendMessage(request);
        return ResponseEntity.noContent().build();
    }

    @GetMapping(path = "/{id1}/{id2}")
    public ResponseEntity<List<MessageView>> getConversation(@PathVariable Long id1, @PathVariable Long id2) {
        return new ResponseEntity<>(messageService.getConversation(id1, id2)
                .stream()
