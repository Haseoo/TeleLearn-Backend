...
public class CourseView extends CourseBriefView {
    private final String welcomePageHtmlContent;

    @JsonProperty("areStudentsAllowedToPost")
    private final boolean studentsAllowedToPost;

    private final List<StudentView> allowedStudents;
    ...
    public static CourseView from(Course model) {
        return new CourseView(model.getId(),
                model.getName(),
                TeacherView.from(model.getOwner(), false),
                model.isAutoAccept(),
                model.getWelcomePageHtmlContent(),
                model.isStudentsAllowedToPost(),
